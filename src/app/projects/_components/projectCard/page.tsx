...
export default function ProjectCard({ project, extraClass }: { project: Project, extraClass?: string }) {
  ...
  return (
    <Link href={`/projects/${project.slug}`} onClick={() => { setCurrentProject(project) }} key={project.name as string} className={`${styles.projectCard} ${extraClass && `${styles.projectCard_homePage}`}`}>
